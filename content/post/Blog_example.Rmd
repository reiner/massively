...
description: Explore the housing market activity in the Canton of Zurich over space and time
...
## high resolution spatial dataset on real estate trades
...
As we've also prepared a data set containing information on the stock of housing over the same hexagon-raster, the trading-data can be set into relation with the number of potentially tradable objects. What patterns come to light when the number of trades are set into relation with the number of existing housing units within the same area?

It might be intriguing to find patterns and to provide spatial explanations by recurring to other open data sets, such as open street map.

# Help us understand whether data sets in this new form are useful and save

We are eager to get feedback on whether such detailed single-record data meets user needs. We've alleviated the sensitivness of the highly detailed data. We've conducted several steps (tests for uniquenes and followingly categorization of the variables) to anonymise unique identifiable objects. Help us improve by, test the data for flaws and tell us how we could do better.
...
```{r setup_handänderungen, eval=FALSE, echo=TRUE, message=FALSE, warning=FALSE}
# load packages
library(tidyverse)
library(sf)
library(ggplot2)
